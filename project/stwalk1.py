...
def random_walk(SpaceTimegraph, path_length, rand=random.Random(0), start=None):
    ...
    while len(path) < path_length:
        ...
        if len(G[cur]) > 0:
            ...
            path.append(rand.choices(list(G[cur]),weights=[G[cur][adjnode]['weight'] for adjnode in list(G[cur])])[0])
        else:
            break
    return path
...
def STWalk1(input_direc, output_file, number_restart, walk_length, representation_size, time_step,
    ...
    G_list = [nx.read_gml(input_direc + "/" + str(i*2+1980) + "_2.gml") for i in
...
if __name__ == '__main__':
    ...
    for t in seq:
        ...
        STWalk1(input_direc=direc + "/input_graphs",
                output_file=direc + "/output_stwalkone/spatiotemporal_" + str(time_step*2+1980) + ".stwalkone",
