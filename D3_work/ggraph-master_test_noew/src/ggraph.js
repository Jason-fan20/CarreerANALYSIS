...
var ggraph = (function() {
  ...
  var charge = function(d) {
    if (d.members[0].type === 'comment') {
      return -100 * Math.sqrt(d.members[0].id.length);
    }
    else {
      return -500;
    }
  ...
  //  groups.clear();
	...
	//console.log(graph.member_map);
	 for (var member_id in graph.member_map) {
	 //console.log(graph.member_map[member_id]);
	// graph.member_map[member_id].node
	 }
...
var re_convert = function(temp,graph) {
    var converted = JSON.parse(JSON.stringify(graph))
