...
def root2visualization(root):
    dfs_toviz(root,'')
    tree_data=deal_sequence(total_sequence)
    make_json(tree_data)
##input root_node,ouput data.json, loaded by index.html        
   ###################################viz   
def load_total(name):
...
#    str_list.append('1 2 4')
#    root=construct_tree(str_list)
#    add_sequence(root,'1 2 5')
    
    str_list=load_total(name)
    root=construct_tree(str_list)
    
    ###################################viz
    root2visualization(root)
    ####generate data.json file
    ###################################~~viz
    
    ###################################compress tree
    root1=compress_tree(root)
    
    ###################################~~compress tree
    root2visualization(root1)
main()
#print(1)
#dfs_toviz(root,'')
